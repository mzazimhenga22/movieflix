...
async function firestoreCommit(projectId: string, accessToken: string, body: any, retries = 3): Promise<any> {
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    if (res.status === 429) {
      if (attempt < retries - 1) {
        const delayMs = Math.pow(2, attempt + 1) * 1000; // exponential backoff: 2s, 4s, 8s
        await new Promise((resolve) => setTimeout(resolve, delayMs));
        continue; // retry
      }
      throw new HttpError(429, 'Firestore quota exceeded. Please try again later or upgrade your Firebase plan.');
    }
  ...
  throw new HttpError(429, 'Firestore quota exceeded. Please try again later.');
}
