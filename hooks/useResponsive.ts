...
export function useResponsive(): ResponsiveConfig {
  ...
  return useMemo(() => {
    ...
    const ms = (size: number, factor = 0.4) => {
    ...
    // Show ~2.1 cards on screen for default, giving more breathing room
    const cardGap = isSmallScreen ? 12 : isMediumScreen ? 14 : 16;
    const horizontalPadding = isSmallScreen ? 14 : isMediumScreen ? 18 : 22;
    
    // Calculate card width to fit 2.1 cards with gaps
    const availableWidth = screenWidth - (horizontalPadding * 2);
    const defaultCardWidth = Math.floor((availableWidth - cardGap * 2) / 2.1);
    const cardWidth = Math.max(135, Math.min(defaultCardWidth, 180));
    const cardHeight = Math.round(cardWidth * 1.5); // 2:3 aspect ratio
    
    // Compact cards (smaller screens or grid view)
    const compactCardWidth = Math.max(110, Math.round(cardWidth * 0.8));
    const compactCardHeight = Math.round(compactCardWidth * 1.5);
    
    // Large cards (featured/spotlight)
    const largeCardWidth = Math.min(wp(80), 340);
...
export function getResponsiveCardDimensions(screenWidth: number) {
  ...
  const largeCardWidth = Math.min(screenWidth * 0.80, 340);
  const largeCardHeight = Math.round(largeCardWidth * 1.03);
