...
async function main() {
  ...
  try {
    // We expect a 400 or 401, or 500. A 404 would be bad (function not deployed).
    // Sending a dummy request without auth tokens first to check reachability.
    const res = await fetch(functionUrl, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        // 'apikey': supabaseKey, // Intentionally omitted to see raw response or if it demands auth
