...
export default function FeedCard({
  ...
  useEffect(() => {
    if (autoPlayFeedVideos && active && item.videoUrl) {
      player.play();
    } else {
      player.pause();
    }
  }, [active, autoPlayFeedVideos, item.videoUrl, player]);
  ...
  const showVideoThumb = videoLoading;
        ...
        <Pressable style={styles.mediaWrap} onPress={handleTap}>
          ...
          <View style={styles.bottomOverlay}>
            ...
            {item.musicTrack && (
              ...
              >
                <Image source={{ uri: item.musicTrack.thumbnail }} style={styles.musicArt} />
                <View style={styles.musicArtCenter} />
                <Text style={styles.musicBadgeText} numberOfLines={1}>
                  {item.musicTrack.title} • {item.musicTrack.artist}
                </Text>
              </TouchableOpacity>
            )}
            {item.movie && (
          ...
          {/* Side action bar - Glassmorphic Instagram Reels style */}
          <View style={styles.sideActions}>
            ...
            >
              <LiquidGlass
                glowColor={item.liked ? '#ff2d55' : '#ffffff'}
                tintColor={item.liked ? '#ff2d55' : '#1A1A2E'}
                tintOpacity={0.35}
                cornerRadius={24}
                glowIntensity={0.5}
                borderWidth={1}
                style={styles.sideActionGlass}
                animated={false}
              >
                <Ionicons name={item.liked ? 'heart' : 'heart-outline'} size={26} color={item.liked ? '#ff2d55' : '#fff'} />
              </LiquidGlass>
            ...
            >
              <LiquidGlass
                glowColor={item.bookmarked ? '#ffd700' : '#ffffff'}
                tintColor={item.bookmarked ? '#ffd700' : '#1A1A2E'}
                tintOpacity={0.35}
                cornerRadius={24}
                glowIntensity={0.5}
                borderWidth={1}
                ...
                animated={false}
              >
                <Ionicons name={item.bookmarked ? 'bookmark' : 'bookmark-outline'} size={24} color={item.bookmarked ? '#ffd700' : '#fff'} />
              </LiquidGlass>
            </TouchableOpacity>

            <TouchableOpacity
              style={[styles.sideActionBtn, (chatBusy || !item.userId || isOwnItem) && styles.sideActionDisabled]}
              disabled={chatBusy || !item.userId || isOwnItem}
              onPress={async () => {
                if (chatBusy || !user?.uid || !item.userId || item.userId === user.uid) return;
                setChatBusy(true);
                try {
                  const { findOrCreateConversation, getProfileById } = await import('../../../app/messaging/controller');
            ...
            >
              <LiquidGlass
                glowColor="#22c55e"
                tintColor="#1A1A2E"
                tintOpacity={chatBusy || !item.userId || isOwnItem ? 0.15 : 0.35}
                cornerRadius={24}
                glowIntensity={chatBusy || !item.userId || isOwnItem ? 0.2 : 0.5}
                borderWidth={1}
                style={styles.sideActionGlass}
                animated={false}
              >
                <Ionicons name="chatbubble-ellipses-outline" size={22} color={chatBusy || !item.userId || isOwnItem ? 'rgba(255,255,255,0.4)' : '#fff'} />
              </LiquidGlass>
            </TouchableOpacity>
          </View>
        </Pressable>
