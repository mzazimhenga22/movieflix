...
import type { DocumentData, DocumentReference, QueryDocumentSnapshot, Transaction } from '@firebase/firestore';
import { useCallback, useEffect, useRef, useState } from 'react';
import { Alert, Platform } from 'react-native';
import { firestore } from '../../../constants/firebase';
import { supabase, supabaseConfigured } from '../../../constants/supabase';
import { useUser } from '../../../hooks/use-user';
import { logInteraction, rankFeed } from '../../../lib/algo';
...
export function useSocialReactions() {
  ...
  const refreshReviews = useCallback(async () => {
    ...
    try {
          ...
          snapshot.docs.map(async (docSnap: QueryDocumentSnapshot<DocumentData>) => {
            ...
            try {
              ...
              if (!commentsSnap.empty) {
                comments = commentsSnap.docs.map((commentDoc: QueryDocumentSnapshot<DocumentData>) => {
      ...
      try {
        const ranked = await rankFeed(items, user?.uid ?? 'anon', []);
    ...
    async (id: ReviewItem['id']) => {
      ...
      setReviews((prev) =>
        prev.map((item) => {
          ...
          const nextLikerIds = nextLiked
            ...
            : baseLikerIds.filter((x: string) => x !== uid);
          return {
            ...item,
            liked: nextLiked,
      ...
      try {
        if (targetReview.origin === 'firestore' && targetReview.docId) {
          ...
          const result = await runTransaction(firestore, async (tx: Transaction) => {
  ...
  const handleComment = useCallback((id: ReviewItem['id'], text?: string) => {
    ...
    if (pendingDocId) {
      ...
      const commentPromise = addDoc(commentsRef, payload).then((commentDoc: DocumentReference<DocumentData>) => {
        createNotification({
