...
export default function StartupVideoSplash({ visible, onDone, maxDuration = DEFAULT_MAX_DURATION }: Props) {
  ...
  const bgPlayer = useVideoPlayer(source, (p) => {
    p.loop = false;
    p.muted = true;
  });

  const mainPlayer = useVideoPlayer(source, (p) => {
    p.loop = false;
    p.muted = true;
  });
  ...
  useEffect(() => {
    const sub = mainPlayer.addListener('statusChange', (status) => {
      if (status.status === 'readyToPlay') setVideoLoaded(true);
    });
    const subEnd = mainPlayer.addListener('playToEnd', () => {
      safeDone();
    });
    return () => {
      sub.remove();
  ...
  }, [visible, videoLoaded, bgPlayer, mainPlayer]);

  // Animations initialized above
