...
export default function UpdateGate({ children }: Props) {
  ...
  const handleOpenRemoteUpdate = useCallback(async () => {
    ...
    try {
      const canOpen = await Linking.canOpenURL(prompt.actionUrl);
      if (canOpen) {
        await Linking.openURL(prompt.actionUrl);
      } else {
        console.warn('[UpdateGate] cannot open update URL:', prompt.actionUrl);
      }
    } catch (err) {
      console.warn('[UpdateGate] failed to open update URL:', err);
    }
  }, [prompt]);
