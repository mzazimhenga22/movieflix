...
// Native implementation handles this
const fetchFallbackSubtitles = fetchFallbackSubtitlesNative;
...
const FetchTmdbEnrichment = async (
  ...
  return fetchTmdbEnrichmentNative(tmdbId, mediaType);
};
FetchTmdbEnrichment.displayName = 'FetchTmdbEnrichment';

function needsRgShowsHeaders(uri?: string, sourceId?: string, embedId?: string) {
...
const VideoPlayerScreen = () => {
  ...
  useEffect(() => {
    onHeavyScreenFocus('video-player');
    return () => onHeavyScreenBlur('video-player');
  }, []);
  ...
  const ShowMessageToast = useCallback(
    function ShowMessageToastImpl(next: InPlayerMessageToast) {
      if (messageToastHideTimerRef.current) {
        clearTimeout(messageToastHideTimerRef.current);
        messageToastHideTimerRef.current = null;
      }
  ...
  // eslint-disable-next-line react/display-name
  const EnsureFollowingLoaded = useCallback(async function EnsureFollowingLoadedImpl() {
    if (!uid) return;
  ...
  useEffect(() => {
    const removeListener = navigation.addListener('beforeRemove', (e: any) => {
      // If we are playing and PiP is enabled/supported, preventing exit to enter PiP
      if (isPlayingRef.current && pipUiEnabled && !isPipActiveRef.current && !midrollActiveRef.current) {
        // Only prevent if we can actually enter PiP
        // Resolve PiP handlers again just to be safe or use ref if needed, 
        // though they are memoized.
        const { enterPip } = resolvePipHandlers();
        if (enterPip) {
          e.preventDefault();
          setPipPending(true);
          // Ensure audio continues in background (PiP)
          ensurePlaybackAudioMode(true).then(() => {
            enterPip().catch(() => {
              // If PiP fails, dispatch original action
  ...
  useEffect(() => {
    ...
    const fetchManifest = async () => {
      try {
        ...
        if (cancelled) return;
        setAudioTrackOptions(await parseHlsAudioTracks(text));
        setQualityOptions(await parseHlsQualityOptions(text, manifestUrl));
      } catch (err) {
        if (!cancelled) {
  ...
  useLayoutEffect(() => {
    ...
    const lock = async () => {
      try {
        ...
        if (active) setIsOrientationLocked(true);
      }
    };
    lock();
    return () => { active = false; };
  ...
  // lock orientation maintenance + setup brightness
  useFocusEffect(
    useCallback(() => {
      let cancelled = false;
      let lockActive = true;

      // Ensure we stay landscape if focus returns
        ...
        if (shouldRemove) {
          ...
          if (user?.uid) {
            ...
            if (options.markComplete) {
              const genres = baseEntry.genre_ids ? baseEntry.genre_ids.map(String) : [];
              void checkAndAwardBadges(user.uid, {
                tmdbId: Number(baseEntry.id),
                title: baseEntry.title || baseEntry.name || 'Unknown',
                mediaType: baseEntry.media_type === 'tv' ? 'tv' : 'movie',
                durationMinutes: durationValue / 60000,
                watchedMinutes: positionValue / 60000,
                genres,
                ...
                if (newBadges.length > 0) {
                  Alert.alert(
                    ...
                    `You earned ${newBadges.length} new badge${newBadges.length > 1 ? 's' : ''}! Check the Streaks tab.`,
                  );
                }
              });
            }
        ...
        if (qualityOverrideUri) {
          ...
          setPlaybackSource(
            createPlaybackSource({
              ...
              embedId: playbackSource.sourceId,
            }),
          );
        }
        setVideoReloadKey((prev) => prev + 1);
...
// JS fallback for caption parsing
function parseCaptionPayloadJS(payload: string, type: 'srt' | 'vtt'): CaptionCue[] {
...
// Try native module first, fallback to JS
async function parseCaptionPayload(payload: string, type: 'srt' | 'vtt'): Promise<CaptionCue[]> {
  try {
    return await parseCaptionPayloadNative(payload, type);
  } catch {
    return parseCaptionPayloadJS(payload, type);
  }
}
...
// JS fallback for HLS audio tracks
function parseHlsAudioTracksJS(manifest: string): AudioTrackOption[] {
...
// Try native module first, fallback to JS
async function parseHlsAudioTracks(manifest: string): Promise<AudioTrackOption[]> {
  try {
    return await parseHlsAudioTracksNative(manifest);
  } catch {
    return parseHlsAudioTracksJS(manifest);
  }
}
...
// JS fallback for HLS quality options
function parseHlsQualityOptionsJS(manifest: string, manifestUrl: string): QualityOption[] {
  const lines = manifest.split('\n');
...
// Try native module first, fallback to JS
async function parseHlsQualityOptions(manifest: string, manifestUrl: string): Promise<QualityOption[]> {
  try {
    return await parseHlsQualityOptionsNative(manifest, manifestUrl);
  } catch {
    return parseHlsQualityOptionsJS(manifest, manifestUrl);
  }
}
...
// JS fallback for HLS media segments
function parseHlsMediaSegmentsJS(
  manifestText: string,
  manifestUrl: string,
...
// Try native module first, fallback to JS
async function parseHlsMediaSegments(
  manifestText: string,
  manifestUrl: string,
): Promise<Array<{ uri: string; duration: number | null }>> {
  try {
    return await parseHlsMediaSegmentsNative(manifestText, manifestUrl);
  } catch {
    return parseHlsMediaSegmentsJS(manifestText, manifestUrl);
  }
}
...
async function preloadStreamWindow(
  ...
  try {
    ...
    const segments = await parseHlsMediaSegments(variantText, variantUrl);
