...
const SearchStickyHeader = memo(({
  ...
  return (
    <View style={[styles.stickyHeader, { paddingTop: insets.top }]}>
      ...
      {/* Search Input */}
      <AnimatedView entering={FadeIn.delay(100).duration(400)} style={styles.searchContainer}>
        <LiquidGlass
          glowColor={accentColor}
          tintColor="#0d0d12"
          tintOpacity={0.6}
          ...
          borderWidth={1.2}
          animated={true}
          style={styles.searchLiquidGlass}
        >
...
SearchStickyHeader.displayName = 'SearchStickyHeader';
...
SearchStateView.displayName = 'SearchStateView';

const SocialItem = memo(({ item }: { item: any }) => (
  <TouchableOpacity activeOpacity={0.85} style={{ marginBottom: 12 }}>
    <LiquidGlass
      glowColor={item.type === 'story' ? '#e50914' : item.type === 'notification' ? '#ffc107' : '#7C3AED'}
      tintColor="#0d0d12"
      tintOpacity={0.55}
      cornerRadius={16}
      glowIntensity={0.5}
    ...
    >
      <View style={styles.socialContentInner}>
        <View
          style={[
            styles.socialIcon,
            {
...
const SearchScreen = () => {
  ...
  useEffect(() => {
    ...
    const searchMovies = async () => {
      // Use controller only for signalling local state if needed, but native module handles request.
      // We can't easily abort native module request yet without passing signal ID, 
      // but simplistic approach is just ignore result if requestId changed.

      try {
        const jsonString = await MoviesModule.searchContent(
          API_KEY,
          API_BASE_URL,
          searchQuery
        );
        ...
        const results: Media[] = JSON.parse(jsonString || '[]');

        // Native module already interleaves movies and TV.
        // We might want to separate them if the UI requires it, 
        // but the current UI uses `movieResults` and `tvResults`.
        // The native `searchContent` returns a combined interleaved list.
        // We can split them back if needed, or update state to use combined.

        // Current UI separates them in state: setMovieResults, setTvResults.
        // And `combinedResults` merges them. 
        // Let's filter here to maintain compatibility with existing state structure 
        // or just put movies in movieResults and tv in tvResults.

        const movies = results.filter(m => m.media_type === 'movie');
        const tvs = results.filter(m => m.media_type === 'tv');

        const moviesFiltered = filterForKidsMedia(movies, isKidsProfile);
        const tvFiltered = filterForKidsMedia(tvs, isKidsProfile);

        setMovieResults(moviesFiltered);
        setTvResults(tvFiltered);
      } catch (err: any) {
        console.warn('Native Search error', err);
