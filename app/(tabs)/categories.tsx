  ...
  return (
    ...
    >
      <LiquidGlass
        ...
        borderWidth={1.5}
        animated={true}
        style={styles.heroLiquidGlass}
      >
        <LinearGradient
          colors={[accentBase + '20', 'rgba(5,6,15,0.85)']}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
...
const CategoriesScreen: React.FC = () => {
  ...
  useEffect(() => {
    const fetchGenres = async () => {
      ...
      try {
        const res = await MoviesModule.fetchGenres(API_KEY, API_BASE_URL, isKidsProfile);
        const incoming = JSON.parse(res) as Genre[];
        setGenres(incoming);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchMoviesByGenre = async () => {
      setIsLoading(true);
      try {
        const res = await MoviesModule.fetchDiscoverMovies(
          API_KEY,
          API_BASE_URL,
          selectedGenre,
          isKidsProfile
        );
        const filtered = JSON.parse(res) as Media[];
