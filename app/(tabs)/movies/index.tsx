...
import SnowOverlay from '@/app/messaging/components/SnowOverlay';
...
import FlixyWalkthrough, { shouldShowWalkthrough } from '@/components/app-components/FlixyWalkthrough';
import LiquidGlass from '@/components/app-components/LiquidGlass';
import StoryCarousel from '@/components/Story.tsx';
...
import FreshDropsOverlay from './components/FreshDropsOverlay';
...
// Global memory cache for filtered collections
const filteredMemoryCache: Record<string, Omit<FilteredCollections, 'becauseYouWatched'>> = {};
...
function useFilteredCollections(params: {
  ...
  // Use a stable key for memory cache
  const cacheKey = useMemo(() => {
    return `${userId || 'anon'}_${activeFilter}_${activeGenreId || 'none'}`;
  }, [userId, activeFilter, activeGenreId]);
  ...
  const [collections, setCollections] = useState<Omit<FilteredCollections, 'becauseYouWatched'>>(() => {
    return filteredMemoryCache[cacheKey] || {
      filteredTrending: [],
      filteredRecommended: [],
      filteredNetflix: [],
      filteredAmazon: [],
    ...
    (async () => {
      try {
        ...
        setCollections((prev) => {
          ...
          if (changed) {
            filteredMemoryCache[cacheKey] = next;
            return next;
          }
          return prev;
        });
...
const HomeScreen: React.FC = () => {
  ...
  const onStickyHeaderLayout = useCallback(
    (e: any) => {
      ...
      // Allow it to shrink or grow, but avoid tiny updates
      setStickyHeaderHeight((prev) => Math.abs(next - prev) < 2 ? prev : next);
    },
    [],
  );
  ...
  // Initialize MyList Store
  useEffect(() => {
    import('@/src/store/myListStore').then(({ myListStore }) => {
      myListStore.init(activeProfileId);
    });
  }, [activeProfileId]);
  ...
  const sections = useMemo<HomeSection[]>(() => {
    ...
    if (filteredTrending.length > 0) out.push({ key: 'top10' });
    if (filteredNetflix.length > 0) out.push({ key: 'netflix' });
    // Editorial breather
    if (filteredRecommended.length > 3) out.push({ key: 'editorial1' });
    if (filteredAmazon.length > 0) out.push({ key: 'amazon' });
    // Duo grid for Top Movies
    if (filteredTrendingMoviesOnly.length >= 2) out.push({ key: 'duoMovies' });
    ...
    // Duo grid for Recommended
    if (filteredRecommended.length >= 2) out.push({ key: 'duoRecommended' });
  ...
  const maxLiquidGlassableSectionIndexRef = useRef(0);
  ...
  const viewabilityConfig = useRef({ viewAreaCoveragePercentThreshold: 35 }).current;
  const prefetchTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const onLiquidGlassableItemsChanged = useRef(
    ({ viewableItems }: { viewableItems: Array<{ index: number | null; isLiquidGlassable: boolean; item: any; key: string }> }) => {
      const maxIndex = viewableItems.reduce((acc, v) => {
        const idx = typeof v.index === 'number' ? v.index : -1;
        return idx > acc ? idx : acc;
      }, -1);
      if (maxIndex >= 0) maxLiquidGlassableSectionIndexRef.current = maxIndex;

      // Debounce prefetch heavily during active scroll
      ...
      }, isListScrollingRef.current ? 800 : 300);
    },
  ).current;

  const estimatedCarouselSectionHeight = useMemo(
    () => Math.round(Math.max(responsiveCards.cardHeight + 140, 360)),
  ...
  const drawDistance = useMemo(
    () =>
      Platform.OS === 'android'
        ? Math.max(2200, Math.round(screenHeight * 2.6))
        : Math.max(1800, Math.round(screenHeight * 2.2)),
    [screenHeight],
  );
  ...
  const renderSection: ListRenderItem<HomeSection> = useCallback(
    ({ item, index }) => {
      ...
      switch (item.key) {
        ...
        case 'editorial1':
          content = filteredRecommended.length > 3 ? (
            <EditorialSection
              movie={filteredRecommended[Math.floor(filteredRecommended.length / 3)]}
        ...
        case 'sciFiPicks':
          content = (
            <GenreShowcaseSection
              title="Sci-Fi Marvels"
              icon="🚀"
              themeColors={['#00bfff', '#0077ff']}
              movies={sciFiPicks}
              onItemPress={handleOpenDetails}
        ...
        <Animated.View style={[styles.contentWrap, screenFxStyle]} pointerEvents={reelsCollapsing ? 'none' : 'auto'}>
          ...
          <View style={[styles.stickyHeaderWrap, { top: STICKY_HEADER_TOP }]} onLayout={onStickyHeaderLayout}>
            <HomeHeader
              ...
              metaRowAnim={metaRowAnim}
              trendingPillScale={trendingPillScale}
              reelsPillScale={reelsPillScale}
              dropsPillScale={dropsPillScale}
              showPulseCards={showPulseCards}
              snowing={snowing}
              scrollY={scrollY}
              onProfilePress={() => deferNav(() => router.push('/profile'))}
              onMessagingPress={() => deferNav(() => router.push('/messaging'))}
              onMarketplacePress={() => deferNav(() => router.push('/marketplace'))}
              onSocialFeedPress={() => deferNav(() => router.push('/social-feed'))}
              onTrendingPress={() => {
              ...
              onPulseToggle={() => setShowPulseCards((v) => !v)}
              onSnowToggle={() => setSnowing((v) => !v)}
            />
            <HomeDock
              featuredAccent={featuredAccent}
              snowing={snowing}
            ...
            style={[
              {
                ...
                left: 0,
                right: 0,
                height: stickyHeaderSpacerHeight + insets.top + 10,
                zIndex: 35,
          ...
          <AnimatedFlashList
            ...
            estimatedItemSize={estimatedCarouselSectionHeight}
            estimatedListSize={{ width: screenWidth, height: screenHeight }}
            drawDistance={drawDistance}
            removeClippedSubviews={false}
            decelerationRate={Platform.OS === 'ios' ? 'normal' : 0.99}
            ...
            contentContainerStyle={styles.scrollLiquidGlassContent}
            ListHeaderComponent={
              <>
                ...
                {showPulseCards ? (
                  <View style={styles.pulseRow}>
                    {cinematicPulse.map((stat) => (
                      <View key={stat.label} style={styles.pulseCard}>
                        <LiquidGlass
                          tintOpacity={0.06}
                          tintColor="#0a0a0a"
                          cornerRadius={18}
                        ...
                        <View style={styles.pulseMeter}>
                          <View
                            style={[styles.pulseMeterFill, { width: `${Math.max(stat.progress * 100, 8)}%`, backgroundColor: stat.palette[1] }]}
                          />
                        </View>
                      </View>
                    ))}
                ...
                {genres.length > 0 && (
                  ...
                  >
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.genreRow}>
                      ...
                      >
                        <LiquidGlass
                          tintOpacity={activeGenreId == null ? 0.15 : 0.04}
                          tintColor={activeGenreId == null ? (featuredAccent || '#e50914') : '#0a0a0a'}
                          cornerRadius={20}
                          borderOpacity={activeGenreId == null ? 0.20 : 0.08}
                          glowIntensity={activeGenreId == null ? 0.2 : 0}
                      ...
                      {genres.map((g) => (
                        ...
                        >
                          <LiquidGlass
                            tintOpacity={activeGenreId === g.id ? 0.15 : 0.04}
                            tintColor={activeGenreId === g.id ? (featuredAccent || '#e50914') : '#0a0a0a'}
                            cornerRadius={20}
                            borderOpacity={activeGenreId === g.id ? 0.20 : 0.08}
                            glowIntensity={activeGenreId === g.id ? 0.2 : 0}
                            glowColor={featuredAccent || '#e50914'}
                            breathingEffect={false}
                            interactive={false}
                            optimizeForScroll
                  ...
                  >
                    {FILTER_KEYS.map((key) => {
                      ...
                      return (
                        ...
                        >
                          <LiquidGlass
                            tintOpacity={isActive ? 0.15 : 0.04}
                            tintColor={isActive ? (featuredAccent || '#e50914') : '#0a0a0a'}
                            cornerRadius={20}
                            borderOpacity={isActive ? 0.20 : 0.08}
                            glowIntensity={isActive ? 0.2 : 0}
                            glowColor={featuredAccent || '#e50914'}
                            breathingEffect={false}
                            interactive={false}
                            optimizeForScroll
          ...
          {/* Sub FABs with animated radial entrance */}
          {fabItems.map((it, idx) => {
            // Increase radius for more spacing
            const radius = 135;
            // Adjust angle spread: from 100 degrees (mostly up) to 170 degrees (mostly left)
            const startAngle = Math.PI / 1.8; // ~100 deg
            const endAngle = Math.PI * 0.95;  // ~170 deg
            const angle = startAngle + (idx * (endAngle - startAngle)) / (fabItems.length - 1);
            
            const tx = fabRotateAnim.interpolate({
              inputRange: [0, 1],
              outputRange: [0, Math.cos(angle) * radius],
            });
            const ty = fabRotateAnim.interpolate({
              inputRange: [0, 1],
              outputRange: [0, -Math.sin(angle) * radius],
            });
            const opacity = fabRotateAnim.interpolate({
              inputRange: [0, 0.4, 1],
              outputRange: [0, 0, 1],
            });
            const scale = fabRotateAnim.interpolate({
              inputRange: [0, 0.7, 1],
            ...
            return (
              ...
              >
                <TouchableOpacity
                  onPress={() => {
                    handleFabPress(); // toggle close
                    setTimeout(() => it.onPress(), 150);
                  }}
                  activeOpacity={0.8}
                ...
                >
                  <LiquidGlass
                    ...
                    style={StyleSheet.absoluteFill}
                  />
                  <Ionicons name={it.icon as any} size={20} color="#FFFFFF" style={{ zIndex: 2 }} />
                </TouchableOpacity>
              </Animated.View>
            );
          })}
          ...
          >
            ...
            >
              <LiquidGlass
                glowColor={featuredAccent || '#06b6d4'}
                tintOpacity={0.35}
                tintColor={featuredAccent || '#06b6d4'}
                cornerRadius={30}
                glowIntensity={0.6}
                borderOpacity={0.4}
                style={StyleSheet.absoluteFill}
