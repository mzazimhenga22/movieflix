...
export default function SplashScreen() {
    ...
    try {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        return {
          ...
          summary: `${parsed.length} saved title${parsed.length === 1 ? '' : 's'} for ${profile.name || 'you'
            }`,
        };
      }
      return null;
  ...
  useEffect(() => {
    ...
    const failsafe = setTimeout(() => {
      if (routePlan || hasNavigatedRef.current) return;
      setOfflineSummary(null);
      setStatusMessage('Starting in guest mode');
      setRoutePlan({ target: '/(auth)/login', context: 'guest', summary: 'Using guest access while startup sync completes.' });
    }, SPLASH_FAILSAFE_MS);
    return () => {
