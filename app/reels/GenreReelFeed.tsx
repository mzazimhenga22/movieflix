...
export const GenreReelFeed = React.memo(function GenreReelFeed({
        ...
        const fetchGenre = async () => {
            try {
                ...
                if (prefetchedClips.length > 0) {
                    ...
                    // 3. Fallback: Fast Lazy Fetch via Native Module
                    // Instead of waiting for streams, just get titles/posters
                    const clipsJson = await ReelsModule.browseGenreMoviesLazy(genreSlug, 10);
                    const clips = JSON.parse(clipsJson) as Array<{ title: string; year: string; slug: string; id: string }>;
                    if (clips.length > 0) {
    ...
    useEffect(() => {
        const resolveUrl = async (index: number) => {
            ...
            try {
                // Determine year if available - use Native Module
                const resultJson = await ReelsModule.searchClipCafe(meta.title, meta.year || '');
    ...
    const onViewableItemsChanged = useRef(
        ({ viewableItems }: { viewableItems: Array<{ index: number | null; isViewable: boolean; item: ReelItem; key: string }> }) => {
    ...
    return (
        <View style={styles.wrapper}>
            <FlatList
                ref={listRef}
                data={items}
                keyExtractor={(it: ReelItem) => String(it.id)}
                ...
                getItemLayout={(_: ReelItem[] | null | undefined, idx: number) => ({
