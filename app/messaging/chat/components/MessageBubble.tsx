...
const MessageBubble = ({
      ...
      void (async () => {
        try {
          ...
          if (canOpen) {
            ...
            console.warn('[MessageBubble] cannot open link:', url);
          }
        } catch (err) {
          console.warn('[MessageBubble] failed to open link:', err);
        }
      })();
    }
  };
  ...
  const [audioSound, setAudioSound] = useState<Audio.AudioSound | null>(null);
      ...
      if (!audioSound) {
        ...
        if (info.exists) {
          ...
          if (!netState.isConnected) {
            ...
            Alert.alert('Offline', 'Connect to the internet to load this track.');
            return;
          }
  ...
  const toggleAudio = React.useCallback(async () => {
    ...
    try {
      if (!audioSound) {
        const { sound } = await Audio.Sound.createAsync({ uri: item.mediaUrl }, {}, (status: Audio.AudioStatus) => {
          if (!status.isLoaded) return;
  ...
  // Premium glassy bubble with LiquidGlass
  const Bubble = (
    <Animated.View style={{ transform: [{ scale: scaleAnim }], opacity: opacityAnim }}>
      <View style={[styles.bubbleOuter, bubbleRadius, isMe ? styles.meShadow : styles.otherShadow]}>
        ...
        {/* Multi-layer gradient for depth - sent messages */}
        {isMe && (
          <>
            <LinearGradient
            ...
            {/* Bottom glow accent */}
            <LinearGradient
              colors={['transparent', withAlpha(accent, 0.15)]}
              locations={[0.7, 1]}
              style={[StyleSheet.absoluteFill, bubbleRadius]}
            />
          </>
        ...
        {!isMe && (
          <>
            ...
            {/* Subtle inner glow */}
            <LinearGradient
              colors={['transparent', withAlpha('#fff', 0.03)]}
              locations={[0.5, 1]}
              style={[StyleSheet.absoluteFill, bubbleRadius]}
            />
          </>
        ...
        {/* Top highlight sheen for glass effect */}
    ...
    <View style={[styles.row, wrapSpacingStyle, isMe ? styles.rowRight : styles.rowLeft]}>
      ...
      <View style={styles.bubbleWrap}>
        <Pressable
          ...
          delayLongPress={130}
          style={({ pressed }: { pressed: boolean }) => [{ transform: [{ scale: pressed ? 0.98 : 1 }] }]}
        >
          {Bubble}
        </Pressable>
