...
const ChatScreen = () => {
  ...
  const handleScroll = (e: any) => {
    ...
    try {
      ...
      const atTop = contentOffset.y <= 80;
  ...
  const visibleMessages = useMemo<ChatMessage[]>(() => {
    ...
    return merged
      ...
      .sort((a, b) => {
        ...
        // Sort oldest-first so newest appears at the bottom (non-inverted list)
  ...
  }, [conversationIdStr, hasMoreMessages, isLoadingMore, visibleMessages]);
              ...
              {isSearchMode && (
                <View style={styles.inlineSearchPanel}>
                  {searchQuery.trim().length === 0 ? (
                    ...
                    <FlatList
                      data={searchResults}
                      keyExtractor={(item: any, index: number) => item.id ?? item.clientId ?? `result-${index}`}
                      ...
                      <FlashList
                        ...
                        renderItem={({ item, index }: { item: ChatMessage; index: number }) => (
                          <ChatRow
                            ...
                            otherPresenceState={otherPresence?.state || 'offline'}
                            directOtherUserId={directOtherUserId}
                            resolveDisplayName={resolveDisplayName}
                            resolveAvatarUri={resolveAvatarUri}
                            getBubbleGroupPosition={getBubbleGroupPosition}
                            onSwipeReply={setReplyTo}
                            onLongPress={(msg, rect, groupPos) => {
                              setSelectedMessage(msg);
                              setSelectedMessageRect(rect);
                              setSelectedMessageGroupPosition(groupPos);
                            }}
                            onPressCall={(msg) => setCallSheetMessage(msg)}
