...
export const onAuthChange = (callback: AuthCallback): UnsubscribeFn => {
  ...
  void authPromise
    .then((auth) => {
      unsubAuth = onAuthStateChanged(auth, (user: any) => {
...
export const onConversationUpdate = (
  ...
  const unsubscribe = onSnapshot(
    ...
    (err: any) => console.error('[messagesController] onConversationUpdate snapshot error:', err),
  )
  return () => unsubscribe()
}
...
export const onConversationsUpdate = (
  ...
  void authPromise
    .then(async (auth) => {
      ...
      // Try cache first
      try {
        const cacheKey = `conversations:${uid}`;
        const cached = await NativeCache.getItem(cacheKey);
        if (!networkReplied && cached) {
      ...
      const snapUnsub = onSnapshot(
        ...
        (querySnapshot: any) => {
          networkReplied = true
          const conversations: Conversation[] = querySnapshot.docs.map((d: any) => ({
            id: d.id,
            ...(d.data() as any),
          }))
          ...
          // Update native cache
          const cacheKey = `conversations:${uid}`
          NativeCache.setItem(cacheKey, JSON.stringify(sorted)).catch(() => { })
        },
        (err: any) => console.error('[messagesController] onConversationsUpdate snapshot error:', err),
      )
...
export const onMessagesUpdate = (
  ...
  const unsubscribe = onSnapshot(
    ...
    (querySnapshot: any) => {
      networkReplied = true
      const messages: Message[] = querySnapshot.docs.map((docSnap: any) => ({
        id: docSnap.id,
        ...(docSnap.data() as any),
      }))
      ...
      // Update cache
      NativeCache.setItem(`messages:${conversationId}`, JSON.stringify(messages)).catch(() => { })
    },
    (err: any) => console.error('[messagesController] onMessagesUpdate snapshot error:', err),
  )
...
export const getFollowing = async (): Promise<Profile[]> => {
  ...
  for (const ids of chunks) {
    ...
    const snap = await getDocs(q)
    results.push(...snap.docs.map((d: any) => ({ ...(d.data() as any), id: d.id } as Profile)))
  }
...
export const getSuggestedPeople = async (): Promise<Profile[]> => {
  ...
  const candidates: (Profile & { followerCount?: number })[] = snapshot.docs
    ...
    .filter((p: any) => p.id !== auth.currentUser!.uid && !followingIds.includes(p.id))
