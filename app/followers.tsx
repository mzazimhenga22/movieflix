...
const FollowersScreen = () => {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const ids = await getFollowersIds(targetUserId);
      const profiles = await fetchProfilesByIds(ids);
      setItems(profiles);
    } catch (e) {
      console.warn('[followers] failed to load', e);
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const renderRow = ({ item }: { item: SocialProfile }) => {
    ...
    return (
      <TouchableOpacity
        style={styles.row}
        activeOpacity={0.85}
        onPress={() => router.push({ pathname: '/profile', params: { userId: item.id, backTo: '/followers' } } as any)}
      >
        {item.photoURL ? (
        ...
        {isSelf ? (
          ...
          <TouchableOpacity
            style={[
              ...
              !isFollowing && { backgroundColor: accentColor || '#e50914' },
              busyId === item.id && { opacity: 0.65 },
            ]}
            onPress={() => void toggleFollow(item)}
            disabled={busyId === item.id}
            activeOpacity={0.85}
          >
            <Text style={styles.followBtnText}>{isFollowing ? 'Following' : 'Follow back'}</Text>
          </TouchableOpacity>
        )}
      </TouchableOpacity>
    );
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={() => void load()} style={styles.refreshBtn}>
            <Ionicons name="refresh" size={20} color={accentColor || '#fff'} />
          </TouchableOpacity>
        </View>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <Text style={styles.emptyText}>{search.trim() ? 'No matches.' : 'No followers yet.'}</Text>
            }
          />
        )}
      </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  followingBtn: { backgroundColor: 'rgba(255,255,255,0.10)' },
