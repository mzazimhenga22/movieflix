...
import { getDownloadSpeed, setDownloadSpeed } from '@/lib/downloadManager';
...
const SettingsScreen: React.FC = () => {
  ...
  const [open, setOpen] = useState<{ movies: boolean; social: boolean; downloads: boolean; flixy: boolean }>({ movies: true, social: false, downloads: false, flixy: false });
  ...
  const downloadSpeedOptions = useMemo(
    () => [
      {
        key: 'high' as const,
        title: 'High speed',
        subtitle: 'Faster downloads (more heat and battery use).',
      },
      {
        key: 'medium' as const,
        title: 'Medium speed',
        subtitle: 'Balanced speed with reduced heat.',
      },
    ],
    [],
  );
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setDownloadSettings(nextDownload);
  ...
  const setDownloadSpeedChoice = useCallback(async (speed: DownloadSettings['speed']) => {
    setDownloadSettings((prev) => ({ ...prev, speed }));
    try {
      await setDownloadSpeed(speed);
    } catch (err) {
      console.warn('Failed to persist download speed', err);
    }
  }, []);
  ...
  const resetDownloadDefaults = useCallback(async () => {
    try {
      await setDownloadSpeed(DEFAULT_DOWNLOAD_SETTINGS.speed);
      setDownloadSettings(DEFAULT_DOWNLOAD_SETTINGS);
    } catch (err) {
      console.warn('Failed to reset download settings', err);
      Alert.alert('Error', 'Failed to reset download settings.');
    }
  }, []);
      ...
      <ScrollView contentContainerStyle={styles.content}>
        {loading ? (
          ...
          <>
            ...
            <SectionCard
              title="Downloads"
              subtitle="Speed preferences"
              isOpen={open.downloads}
              onToggle={() => setOpen((prev) => ({ ...prev, downloads: !prev.downloads }))}
              onReset={() => void resetDownloadDefaults()}
            >
              {downloadSpeedOptions.map((option) => {
                const active = downloadSettings.speed === option.key;
                return (
                  <TouchableOpacity
                    key={option.key}
                    style={[styles.row, active ? styles.rowActive : null]}
                    onPress={() => void setDownloadSpeedChoice(option.key)}
                    activeOpacity={0.85}
                  >
                    <View style={styles.rowText}>
                    ...
                    <View style={[styles.speedBadge, active ? styles.speedBadgeActive : null]}>
                      <Text style={[styles.speedBadgeText, active ? styles.speedBadgeTextActive : null]}>
...
const styles = StyleSheet.create({
  ...
  speedBadgeText: { color: 'rgba(255,255,255,0.8)', fontSize: 12, fontWeight: '600' },
  speedBadgeTextActive: { color: '#fff' },
  flixyNote: {
    flexDirection: 'row',
