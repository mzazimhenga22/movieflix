...
let currentHeavyScreen: string | null = null;
...
function handleAppStateChange(nextAppState: AppStateStatus): void {
    ...
    if (wasBackground && isNowActive && !hasStartedPrefetch) {
        ...
        setTimeout(() => {
            if (appState === 'active' && !currentHeavyScreen) {
...
export function triggerIdlePrefetch(source: string): void {
    if (currentHeavyScreen) {
        console.log('[BackgroundScheduler] Prefetch blocked (heavy screen):', currentHeavyScreen);
        return;
    }
...
export function onHeavyScreenBlur(screenName: string): void {
    if (currentHeavyScreen === screenName) {
        currentHeavyScreen = null;
    }
}
...
export function shouldDeferWork(): boolean {
    return appState !== 'active' || Boolean(currentHeavyScreen);
}
