...
import { firestore, isFirestoreQuotaError } from '../constants/firebase';
...
export const getFollowingIds = async (viewerId: string): Promise<string[]> => {
  ...
  try {
    ...
    if (isFirestoreQuotaError(err)) {
      console.warn('[followGraph] Firestore quota exceeded - returning empty following list');
      return [];
    }
    throw err;
  }
...
export const getFollowersIds = async (userId: string): Promise<string[]> => {
  ...
  try {
    ...
    if (isFirestoreQuotaError(err)) {
      console.warn('[followGraph] Firestore quota exceeded - returning empty followers list');
      return [];
    }
    throw err;
  }
...
export const fetchProfilesByIds = async (ids: string[]): Promise<SocialProfile[]> => {
  ...
  try {
    ...
    if (isFirestoreQuotaError(err)) {
      console.warn('[followGraph] Firestore quota exceeded - returning empty profiles');
      return [];
    }
    throw err;
  }
...
export const followUser = async (options: {
  ...
  try {
    ...
    if (isFirestoreQuotaError(err)) {
      console.warn('[followGraph] Firestore quota exceeded - follow operation failed');
      return { didFollow: false };
    }
    throw err;
  }
...
export const unfollowUser = async (options: {
  ...
  try {
    ...
    if (isFirestoreQuotaError(err)) {
      console.warn('[followGraph] Firestore quota exceeded - unfollow operation failed');
      return { didUnfollow: false };
    }
    throw err;
  }
