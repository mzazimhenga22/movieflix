...
import AlgorithmModule from '../modules/AlgorithmModule';
...
export async function updateTasteProfile(actorId: string): Promise<TasteProfile> {
  ...
  try {
    const profileJson = await AlgorithmModule.computeTasteProfile(JSON.stringify(events));
    const profile = JSON.parse(profileJson);
    await AsyncStorage.setItem(TASTE_PROFILE_KEY, profileJson);
    return profile;
  } catch (e) {
    console.warn('Native Taste Profile Compute Failed', e);
    // Fallback to empty or previous?
    return { genres: {}, keywords: {}, actors: {}, updatedAt: Date.now() };
  }
}
...
export async function recommendContent(items: any[], userId?: string | null) {
  ...
  try {
    const resultJson = await AlgorithmModule.recommend(
    ...
    console.warn('Native Recommendation Failed', e);
    // Fallback: return unsorted/unscored
    return items;
  }
}
