...
async function backgroundFetchGenre(genre: string, movieCount: number, clipsPerMovie: number = 3): Promise<CachedClip[]> {
    ...
    for (let i = 0; i < pathsToFetch.length; i++) {
        ...
        // Try to fetch a high-res trailer/featurette for this movie (mix it in)
        // Only do this for every other movie to save resources OR if we didn't find many clips
        if ((i % 2 === 0 || movieClips.length < 2) && movieClips.length > 0) {
            try {
                const title = movieClips[0].title;
                // movieClips[0].id format: genre-path-idx-timestamp. path often contains year.

                // Simple TMDB Search
                const searchUrl = `${API_BASE_URL}/search/movie?api_key=${API_KEY}&query=${encodeURIComponent(title)}&include_adult=false`;
                ...
                if (searchRes.ok) {
                    ...
                    if (movie) {
                        // Get standard details for IMDB ID
                        const detailUrl = `${API_BASE_URL}/movie/${movie.id}?api_key=${API_KEY}`;
                        const detailRes = await fetch(detailUrl);
                        if (detailRes.ok) {
                            const details = await detailRes.json();
                            if (details.imdb_id) {
                                const trailer = await scrapeImdbTrailer({ imdb_id: details.imdb_id });
                                if (trailer?.url) {
                                    clips.push({
                                        id: `trailer-${movie.id}-${Date.now()}`,
                                        url: trailer.url,
                                        title: `${title} (Trailer)`,
                                        clipName: 'Official Trailer',
                                        genre: genre,
                                        movieIndex: i,
                                        fetchedAt: Date.now(),
                                        headers: undefined,
                                    ...
                                    console.log(`[PrefetchCache] Added trailer for ${title}`);
                                }
                            }
                        }
                    }
