import { NativeCache } from './nativeCache';
...
export async function getPersistedCache<T>(
  ...
  const raw = await NativeCache.getItem(key).catch(() => null);
  if (!raw) return null;
  try {
...
export async function setPersistedCache<T>(key: string, value: T): Promise<void> {
  ...
  await NativeCache.setItem(key, JSON.stringify(envelope)).catch(() => { });
}

export async function deletePersistedCache(key: string): Promise<void> {
  getMemory().delete(key);
  await NativeCache.removeItem(key).catch(() => { });
}
