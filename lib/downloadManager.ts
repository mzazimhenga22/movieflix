...
async function runJob(job: PersistedJob) {
  updateJob(job.sessionId, { status: 'downloading', progress: 0 });
  await saveQueue();
  emit(job, 'downloading', 0);
  ...
  const resumable = FileSystem.createDownloadResumable(
    job.sourceUrl,
    destination,
    job.headers ? { headers: job.headers } : undefined,
    onProgress,
    ...
    while (activeJobs.size < MAX_CONCURRENT) {
      ...
      void (async () => {
        try {
          ...
          await saveQueue();
          emit(job, nextStatus, paused ? job.progress : 0, msg);
          if (cancelled) {
            try {
              if (job.downloadType === 'hls') {
...
export async function pauseDownload(sessionId: string) {
  ...
  try {
    ...
    const state = getAbortController(sessionId);
    state.mode = 'pause';
    updateJob(sessionId, { status: 'paused', resumeData: (resumeData as any) ?? null });
    await saveQueue();
