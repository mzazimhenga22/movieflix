...
export function usePStream() {
  ...
  const getMusicStream = useCallback(async (videoId: string, type: 'audio' | 'video' = 'video', download: boolean = false) => {
    setLoading(true);
    setError(null);
        ...
        try {
          ...
          if (type === 'audio') {
            ...
            if (audioStream?.url) {
              ...
              return { uri: audioStream.url, related };
            }
          } else {
            ...
            if (videoStream?.url) {
              ...
              return { uri: videoStream.url, related };
            }
          }
        } catch (err: any) {
