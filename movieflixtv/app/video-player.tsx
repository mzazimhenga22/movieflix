...
import {
  ...
  parseCaptionPayloadNative,
  parseHlsAudioTracksNative,
  parseHlsMediaSegmentsNative,
  parseHlsQualityOptionsNative,
} from '../modules/VideoPlayerModule';
...
// Native-first subtitle fetch (with JS fallback for non-Android)
const fetchFallbackSubtitles = async (
  imdbId?: string,
  tmdbId?: string,
  mediaType?: string,
  seasonNum?: number,
  ...
  // Prefer native implementation on Android for performance
  if (Platform.OS === 'android') {
    try {
      const nativeResult = await fetchFallbackSubtitlesNative(imdbId, tmdbId, mediaType, seasonNum, episodeNum);
      if (nativeResult && nativeResult.length > 0) {
        return nativeResult as CaptionSource[];
      }
    } catch (e) {
      if (__DEV__) console.warn('[NativeSubs] Failed, using JS fallback', e);
    }
  }
...
// Native-first TMDB enrichment (with JS fallback)
const FetchTmdbEnrichment = async (
  ...
  if (Platform.OS === 'android') {
    try {
      const nativeResult = await fetchTmdbEnrichmentNative(tmdbId, mediaType);
      if (nativeResult && Object.keys(nativeResult).length > 0) {
        return nativeResult as TmdbEnrichment;
      }
    } catch (e) {
      if (__DEV__) console.warn('[NativeEnrichment] Failed, using JS fallback', e);
    }
  }
  
  // JS fallback
  const url = `${API_BASE_URL}/${mediaType}/${tmdbId}?api_key=${API_KEY}&append_to_response=external_ids,release_dates`;
  ...
  return { imdbId, releaseYear, contentRating, runtime: data?.runtime };
};
...
const VideoPlayerScreen = () => {
  ...
  useEffect(() => {
    if (!isTvShow || !isPlaying || skipIntroShown) return;
    if (positionMillis >= introWindow.start && positionMillis <= introWindow.end) {
      setShowSkipIntro(true);
    } else if (positionMillis > introWindow.end) {
      setShowSkipIntro(false);
      if (!skipIntroShown) setSkipIntroShown(true);
    }
  }, [positionMillis, isTvShow, isPlaying, introWindow, skipIntroShown]);

  // Next episode countdown (last 90 seconds)
  useEffect(() => {
    if (!isTvShow || !durationMillis || durationMillis < 60000) return;
    if (!nextUpEpisode) return;

    const remainingMs = durationMillis - positionMillis;
    const isInWindow = remainingMs > 0 && remainingMs <= NEXT_EPISODE_WINDOW_MS;

    if (isInWindow && isPlaying) {
      setShowNextEpisode(true);
      const progress = 1 - (remainingMs / NEXT_EPISODE_WINDOW_MS);
      setNextEpisodeProgress(progress);

      // Prefetch next episode stream in background
      const prefetchKey = `${tmdbId}-${nextUpEpisode.seasonNumber}-${nextUpEpisode.episodeNumber}`;
      if (nextEpisodePrefetchStartedRef.current !== prefetchKey) {
        nextEpisodePrefetchStartedRef.current = prefetchKey;
        // Prefetch will be handled by the episode loading logic
      }
    } else if (!isInWindow) {
      setShowNextEpisode(false);
      setNextEpisodeProgress(0);
    }
  }, [positionMillis, durationMillis, isTvShow, isPlaying, nextUpEpisode, tmdbId]);
  const { user } = useUser();
  ...
  const handleSkipIntro = useCallback(() => {
    if (!player) return;
    const targetPosition = introWindow.end + 1000; // Skip 1 second past intro
    player.currentTime = targetPosition / 1000; // Convert to seconds for expo-video
    setShowSkipIntro(false);
  ...
  const handlePlayNextEpisode = useCallback(async () => {
    if (!nextUpEpisode || !isTvShow || !tmdbId) return;

    const nextSeasonNumber = nextUpEpisode.seasonNumber ?? seasonNumberParam;
    const nextEpisodeNumber = nextUpEpisode.episodeNumber ?? (episodeNumberParam ?? 0) + 1;

    // Shift queue
    setEpisodeQueue(prev => prev.slice(1));
    setShowNextEpisode(false);
    setNextEpisodeProgress(0);
    ...
    const payload = {
      ...
      season: {
        ...
        tmdbId: nextUpEpisode.seasonTmdbId ?? '',
        title: nextUpEpisode.seasonName ?? `Season ${nextSeasonNumber}`,
      },
      episode: {
        number: nextEpisodeNumber,
        tmdbId: nextUpEpisode.episodeTmdbId ?? '',
      },
    };

    try {
      const playback = await scrapeEpisode(payload, { sourceOrder });
      if (!playback.uri) throw new Error('No stream URL');

      const formattedTitle = `${displayTitle} • S${String(nextSeasonNumber).padStart(2, '0')}E${String(nextEpisodeNumber).padStart(2, '0')}`;
      ...
      // Sync to watch party if host
      if (isWatchPartyHost && watchPartyRef) {
        await updateDoc(watchPartyRef, {
          episode: {
            seasonNumber: nextSeasonNumber,
            episodeNumber: nextEpisodeNumber,
            seasonTmdbId: nextUpEpisode.seasonTmdbId ?? null,
            episodeTmdbId: nextUpEpisode.episodeTmdbId ?? null,
            seasonTitle: nextUpEpisode.seasonName ?? null,
            episodeTitle: nextUpEpisode.title ?? null,
            updatedAt: serverTimestamp(),
          },
        }).catch(() => {});
      }
    } catch (err) {
      console.error('[VideoPlayer] Failed to load next episode', err);
      Alert.alert('Next Episode', 'Unable to load the next episode. Please try manually selecting it.');
    }
  }, [
    nextUpEpisode,
    isTvShow,
  ...
  // Reset intro/next episode state when source changes
  useEffect(() => {
    setSkipIntroShown(false);
    setShowSkipIntro(false);
    setShowNextEpisode(false);
    setNextEpisodeProgress(0);
    nextEpisodePrefetchStartedRef.current = null;
  }, [playbackSource?.uri]);
  ...
  const tvRemoteHandler = useCallback(
    (evt: any) => {
      ...
      // CRITICAL: When controls are hidden, ANY button press should wake controls first
      // Don't perform actions - just show controls
      if (!controlsVisible) {
        // Show controls on ANY button press when hidden (except menu/back which should exit)
        if (type !== 'menu' && type !== 'back' && type !== 'exit') {
      ...
      // Controls are visible - allow navigation and actions
      if (controlsVisible) {
        // Navigation buttons are handled by native focus system
      ...
      switch (type) {
        case 'select':
          // Only toggle play/pause if controls are visible and nothing is focused
        ...
        case 'fastForward':
          ...
          return;
        case 'up':
          // Handled by focus system when controls visible
          return;
        case 'down':
          // Handled by focus system when controls visible
          return;
      ...
      <View style={styles.touchLayer}>
        {transitionReady && videoPlaybackSource ? (
          <>
            ...
            {/* Skip Intro Button */}
            {showSkipIntro && !midrollActive && (
              <TvFocusable
                onPress={handleSkipIntro}
                tvPreferredFocus
                style={({ focused }: any) => [
                  styles.skipIntroButton,
                  focused ? styles.skipIntroButtonFocused : null,
                ]}
              >
            ...
            {showNextEpisode && nextUpEpisode && !midrollActive && (
              <View style={styles.nextEpisodeContainer}>
                <View style={styles.nextEpisodeCard}>
                  <Image
                    source={{ uri: nextUpEpisode.stillPath ? `https://image.tmdb.org/t/p/w300${nextUpEpisode.stillPath}` : FALLBACK_EPISODE_IMAGE }}
                    style={styles.nextEpisodeThumbnail}
                  />
                  <View style={styles.nextEpisodeInfo}>
                    <Text style={styles.nextEpisodeTitle}>Up Next</Text>
                    <Text style={styles.nextEpisodeEpisode} numberOfLines={1}>
                      {nextUpEpisode.title ?? `Episode ${nextUpEpisode.episodeNumber}`}
                    </Text>
                  </View>
                  <View style={styles.nextEpisodeProgressCircle}>
                    <Animated.View
                      style={[
                        styles.nextEpisodeProgressFill,
                        { width: `${nextEpisodeProgress * 100}%` },
                      ]}
                    />
                    <TvFocusable
                      onPress={handlePlayNextEpisode}
                      style={({ focused }: any) => [
                        styles.nextEpisodePlayButton,
                        focused ? styles.nextEpisodePlayButtonFocused : null,
                      ]}
                    >
                ...
                <TvFocusable
                  ...
                  style={({ focused }: any) => [
                    styles.nextEpisodeCancelButton,
                    focused ? styles.nextEpisodeCancelButtonFocused : null,
                  ]}
                >
        ...
        {/* TV Wake Indicator - shows when controls are hidden */}
        {!showControls && isTvDevice && !isLocked && !midrollActive && (
          <View style={styles.wakeIndicatorContainer} pointerEvents="none">
            <View style={styles.wakeIndicator}>
              <Ionicons name="tv" size={20} color="rgba(255,255,255,0.6)" />
              <Text style={styles.wakeIndicatorText}>Press any button</Text>
            </View>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  nextEpisodeCard: {
    ...
    padding: 12,
    borderRadius: 12,
    backgroundColor: 'rgba(7,9,18,0.92)',
    borderWidth: 1,
    borderColor: 'rgba(255,255,255,0.15)',
    width: 320,
    gap: 12,
  ...
  nextEpisodeCancelButtonFocused: {
    borderColor: '#fff',
