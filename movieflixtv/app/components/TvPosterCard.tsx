...
function TvPosterCard({
  ...
  const overlayScale = useMemo(() => new Animated.Value(0.95), []);
  ...
  const pulseGlowOpacity = useMemo(() => pulseAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0.4, 1],
  }), [pulseAnim]);

  const pulseGlowScale = useMemo(() => pulseAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [1, 1.15],
  }), [pulseAnim]);
