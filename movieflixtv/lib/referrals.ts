...
const generateReferralCode = (): string => Math.random().toString(36).slice(2, 10).toUpperCase();
...
export const applyReferralCodeOnSignup = async (args: {
  ...
  await runTransaction(firestore, async (tx: Transaction) => {
    ...
    const shouldUpgrade = targetTier !== null && PLAN_RANK[targetTier] > PLAN_RANK[currentTier];
